        ...
        <div className={classes.root}>
            <Card className={type === 'compType' || type === 'conference' ? classes.cardContentLeft:classes.cardContentRight} variant="outlined">
                ...
                <Popover
                    ...
                    {type === 'authors' && d.searchedAuthorsList.map((key) => {
                        return <LazyLoad height={50} offsetVertical={1000}>
                            <MenuItem className={classes.option} selected={d.filterState[type][key]} onClick={() => d.changeFilterState(type, key)} key={key}><ListItemText primary={key}/><Checkbox checked={d.filterState[type][key]}/></MenuItem>
                        </LazyLoad>
                    })}
                    {type === 'compType' ? <div>
                        ...
                        {/* <MenuItem className={classes.optGrp} selected key={'coOccurrence_father'}>{LabelToName['coOccurrence']}</MenuItem>
